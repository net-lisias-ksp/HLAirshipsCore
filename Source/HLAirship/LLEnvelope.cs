...
public class LLEnvelopePartModule : PartModule
{
    ...
    private List<LLEnvelopePartModule> Envelopes = new List<LLEnvelopePartModule>();
    private Part leadEnvelope;
    private LLEnvelopePartModule envelope;
    ...
    {
        foreach (LLEnvelopePartModule envelope in Envelopes)
        {
    ...
    {
        Debug.Log("LLA: Sending guiToggle signal");
    ...
    {
        Debug.Log("LLA: Executing guiToggle method");
		guiOn = !guiOn;
    ...
    public void findEnvelopes()
    {
        ...
        foreach (Part part in this.vessel.parts)
        {
            if (part.isAttached)
            {
                ...
                if (part.Modules.OfType<LLEnvelopePartModule>().FirstOrDefault() == null)
                {
                    // Debug.Log("Did not find an envelope on " + part.name);
                    continue;
                }
                envelope = part.Modules.OfType<LLEnvelopePartModule>().FirstOrDefault();
    ...
    public float autoPitchControl(Vector3 position, LLEnvelopePartModule envelope)
    {
    ...
    public void updateTargetSpecificVolumeFraction(float fraction, LLEnvelopePartModule envelope)
    {
    ...
    private float AngleDir(Vector3 fwd, Vector3 targetDir, Vector3 up)
    {
        ...
        return Math.Sign(Vector3.Dot(perp, up));
    }
